...
from run_config import edge_weights
...
def calc_noise(file_path: str, agent_number: int) -> dict:
    ...
    for round_number, seeds_per_round in seeds.items():
        ...
        for i, seed in enumerate(seeds_per_round):
            ...
            noise_adjustment = np.random.randint(M + 1, size=162, dtype=np.int64)

            client_edge_weight = edge_weights[i]  # Fetch the weight for the ith agent
            if run_config.QUANTISATION:
                noise_vector = modulus(noise_vector + (client_edge_weight * (run_config.server_config["num_clients"] - 1) * noise_adjustment if i == agent_number else -client_edge_weight * noise_adjustment))
            else:
                uniform_noise = np.round(np.random.uniform(-2, 2, 162), 5)
                noise_vector = noise_vector.astype(float)
                noise_vector += client_edge_weight * (uniform_noise * float(run_config.server_config["num_clients"] - 1 if i == agent_number else -1))
